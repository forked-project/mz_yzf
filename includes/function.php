...
function update_version($access_token,$version)
{
	$string = authcode($_SERVER["HTTP_HOST"] . "||||" . $access_token . "||||" . $version, "ENCODE", "muzhifu_yun1");
	$query = curl_get("https://api.muzhifu.cc/Yizf/getcms.php?string=". urlencode($string));
	$query = authcode($query, "DECODE", "muzhifu_yun2");
...
function send_sms($phone, $code, $moban='1'){
	global $conf;
	$appname=$conf['web_name'];
	$url = 'https://api.smszx.cn/index.php/Api/Send?uid='.$conf['sms_uid'].'&access_token='.$conf['sms_token'].'&mobile='.$phone.'&smsid='.$moban.'&appname='.$appname.'&code='.$code;
	$data=get_curl($url);
